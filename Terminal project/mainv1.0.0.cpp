...
std::string getOperationInput(const std::vector<double>& results, const std::vector<std::string>& expressions) {
    std::string op;
    while (true) {
        std::cout << "Enter operation (+, -, *, /, ^2, ^, r, root): ";
        std::cin >> op;
        ...
        if (op == "v") {
            ...
            return "";  // Return an empty string to prevent further action
        } else if (op == "+" || op == "-" || op == "*" || op == "/" ||
...
std::pair<std::vector<double>, std::vector<std::string> > getMultipleNumbersAndOps(
    ...
    std::cout << "Use #n to reference stored results (e.g., #1, #2):\n";
    ...
    while (true) {
        ...
        if (op == "") break;  // Skip adding operations if 'v' was pressed.

...
void drawHUD(const std::vector<double>& results, const std::vector<std::string>& expressions) {
    ...
    std::cout << "\n  ";
    drawButton("sin"); drawButton("cos"); drawButton("#n"); drawButton("=");
    std::cout << "\n\n  ";
    drawButton("View All (v)"); drawButton("Done (d)"); drawButton("Exit (n)");
    std::cout << "\n\n";
}
...
std::string getOperationInput(const std::vector<double>& results, const std::vector<std::string>& expressions) {
    std::string op;
    while (true) {
        std::cout << "Enter operation (+, -, *, /, ^2, ^, r, root, sin, cos), 'v' to view all, or 'done' to finish: ";
        std::cin >> op;

...
std::vector<double> getMultipleNumbers(const std::vector<double>& results) {
...
int main() {
    ...
    while (std::tolower(cont) == 'y') {
        std::vector<double> numbers = getMultipleNumbers(results);

        char op = getCharInput("Enter operation (+, -, *, /)", results, expressions);
        ...
        cont = getCharInput("\nDo you want to continue?", results, expressions);
    }
